...
(function(root, mod) {
  ...
  function updateText(file, text, srv) {
    ...
    // Slava: if this is a template, try to parse it ourselves
    if (file.name.match(/\.html$/)) {
      // For each <template name="bla"> tag
      var re = /<template[^>]+name="([^"]+)"[^>]*>/g;
      var match;
      var newScript = "";
      while (match = re.exec(file.text)) {
        newScript += "Template['" + match[1] + "'] = new Template();\n";
      }
  ...
  var Server = exports.Server = function(options) {
    ...
    for (var plugin in options.plugins) if (options.plugins.hasOwnProperty(plugin) && plugin in plugins) {
      ...
      if (init && init.defs) {
        init.defs = filteredDefs(init.defs, this.options.type);
  ...
  function doRequest(srv, doc, c) {
    ...
    analyzeAll(srv, timeBudget, function(err) {
      ...
      if (! file) return c("File not found in server");
  ...
  function ensureFile(srv, name, parent, text) {
    // Slava: ignore client folder on server and vice-versa
    if (srv.options.type === 'client' && name.match(/(^|\/)server($|\/)/g))
      return;
    if (srv.options.type === 'server' && name.match(/(^|\/)client($|\/)/g))
      return;
  ...
  var resolvePos = exports.resolvePos = function(file, pos, tolerant) {
    ...
    if (pos > file.text.length) {
      if (tolerant) pos = file.text.length;
      else throw ternError("Position " + pos + " is outside of file. file: " + file.name);
    }
    return pos;
  };
...
var filteredDefs = function (defs, type) {
  if (typeof defs !== 'object')
    return defs;
  var matchedType = function (t, type) {
    t = t.toLowerCase();
    if (t === 'anywhere')
      t = type;
    return t === type;
  ...
  var newDefs = {};
  for (var sym in defs) {
    if (! defs.hasOwnProperty(sym))
      continue;
    var val = defs[sym];
    if (typeof val !== 'object' || !val['!locus'] || matchedType(val['!locus'], type))
      newDefs[sym] = filteredDefs(val, type);
  }
  return newDefs;
}
